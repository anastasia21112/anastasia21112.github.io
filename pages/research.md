---
layout: single
title: "Research"
permalink: /research/
classes:
  - wide
  - research-page
author_profile: false
---

<div class="publications">

<h1 class="section-heading">Research Interests</h1>

<div class="research-interests">
  <p>My research sits at the intersection of machine learning and the sciences, with a focus on building models that are both reliable and useful for scientific discovery. Broadly, I am interested in:</p>
  <ul>
    <li><strong>AI safety and robustness</strong> &mdash; evaluation and auditing of large language models in human facing and healthcare settings </li>
    <li><strong>Machine learning for computational biology</strong> &mdash; learning structure from high-dimensional biological data such as single-cell RNA sequencing, and using it to characterize molecular phenotypes.</li>
  </ul>
</div>

<h1 class="publications-heading">Publications</h1>

<h2 class="publication-year">2025</h2>

<div class="publication">
  <img src="/assets/images/mulbere.png" class="publication-thumb" alt="MULBERE figure">
  <div class="publication-info">
    <h3 class="publication-title">MULBERE: Multilingual Jailbreak Robustness Using Targeted Latent Adversarial Training</h3>
    <p class="publication-authors"><strong>Anastasia Dunca</strong>, Maanas Kumar Sharma, Olivia Munoz, Victor Rosales</p>
    <p class="publication-venue"><em>Proceedings of the 9th Widening NLP Workshop (WiNLP), 2025</em></p>
    <p class="publication-abstract">MULBERE extends targeted latent adversarial training to multilingual settings to improve large language model robustness against jailbreak attacks across high-, medium-, and low-resource languages.</p>
    <p class="publication-links">
      <a href="https://aclanthology.org/2025.winlp-main.27/">Paper</a>
      <a href="https://aclanthology.org/2025.winlp-main.27.pdf">PDF</a>
    </p>
  </div>
</div>

<h2 class="publication-year">2024</h2>

<div class="publication">
  <img src="/assets/images/ewaste.png" class="publication-thumb" alt="Electronics lifecycle figure">
  <div class="publication-info">
    <h3 class="publication-title">From Mining to E-waste: The Environmental and Climate Justice Implications of the Electronics Hardware Life Cycle</h3>
    <p class="publication-authors">Lelia Hampton, Madeline Schlegel, Ellie Bultena, Jasmin Liu, <strong>Anastasia Dunca</strong>, Mrinalini Singha, Sungmoon Lim, Lauren Higgins, Christopher Rabe</p>
    <p class="publication-venue"><em>MIT Case Studies in Social and Ethical Responsibilities of Computing, 2024</em></p>
    <p class="publication-abstract">A case study examining environmental and climate justice issues throughout the electronics lifecycle, from mineral extraction and manufacturing to electronic waste.</p>
    <p class="publication-links">
      <a href="https://doi.org/10.21428/2c646de5.2cbc88c6">Paper</a>
    </p>
  </div>
</div>

<h2 class="publication-year">2023</h2>

<div class="publication">
  <img src="/assets/images/scrna.png" class="publication-thumb" alt="Single-cell RNA sequencing figure">
  <div class="publication-info">
    <h3 class="publication-title">Predicting Molecular Phenotypes with Single Cell RNA Sequencing Data: An Assessment of Unsupervised Machine Learning Models</h3>
    <p class="publication-authors"><strong>Anastasia Dunca</strong>, Frederick R. Adler</p>
    <p class="publication-venue"><em>SIAM Undergraduate Research Online, Volume 16, 2023</em></p>
    <p class="publication-abstract">An evaluation of unsupervised machine-learning approaches for identifying molecular phenotypes and tumor heterogeneity from single-cell RNA-sequencing data.</p>
    <p class="publication-links">
      <a href="https://www.siam.org/publications/siam-journals/siam-undergraduate-research-online-siuro/issues/siuro-volume-16/">Paper</a>
      <a href="https://www.siam.org/media/qnlkcgfk/s143998rrr.pdf">PDF</a>
      <a href="https://arxiv.org/abs/2108.05039">arXiv</a>
    </p>
  </div>
</div>

<h2 class="publication-year">2022</h2>

<div class="publication">
  <img src="/assets/images/grain-growth.png" class="publication-thumb" alt="Grain growth figure">
  <div class="publication-info">
    <h3 class="publication-title">Grain Growth and the Effect of Different Time Scales</h3>
    <p class="publication-authors">Katayun Barmak, <strong>Anastasia Dunca</strong>, Yekaterina Epshteyn, Chun Liu, Masashi Mizuno</p>
    <p class="publication-venue"><em>Research in Mathematics of Materials Science, Springer, 2022</em></p>
    <p class="publication-abstract">A mathematical and computational study of grain-boundary dynamics, including the effects of lattice misorientation, triple-junction drag, and multiple time scales.</p>
    <p class="publication-links">
      <a href="https://doi.org/10.1007/978-3-031-04496-0_2">Springer</a>
      <a href="https://arxiv.org/abs/2105.07255">arXiv</a>
    </p>
  </div>
</div>

</div>
